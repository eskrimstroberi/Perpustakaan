...
public void prosesPeminjaman(siswa siswa, buku buku, peminjaman peminjaman) {
        ...
        if (siswa.getStatus(idSiswa) == false) { //false= sedang tidak meminjam
            ...
            do {
                ...
                i++;
                siswa.setidbuku(idSiswa, idBuku);
            ...
            System.out.println("Siswa status adalah = " + Siswa.getStatus(idSiswa));
...
//           +"\t\t"+buku.getHarga(idbukupinjem.get(j))+"\t\t"+jumlah); 
    ...
    public void prosesPengembalian(siswa siswa, buku buku, peminjaman peminjaman) {
        ...
        if (siswa.getStatus(idSiswa) == true) {
            ...
            String temp = "";
            System.out.print("masukkan Id buku=");
            int idBuku2 = masukkan.nextInt();
            do {

                if (siswa.getidbuku(idSiswa) != idBuku2) {
                    System.err.println("Buku yang dipinjam salah!");

                } else if (siswa.getidbuku(idSiswa) == idBuku2) {
			System.err.println("Buku yang dipinjam Benar! silahkan lanjut");
                    idbukukembali.add(idBuku2);
            ...
            System.out.println(" Nama Siswa\t=" + siswa.getNama(idSiswa));
            if (siswa.getidbuku(idSiswa) != idBuku2) {
                System.out.println(" Status \t= GAGAL mengembalikan!");
            } else if (siswa.getidbuku(idSiswa) == idBuku2) {
			 System.out.println(" Status \t= BERHASIL mengembalikan!");
                int total = 0;
